...
def check_numeric_limit(expr, r_sym, vars_dict):
    ...
    try:
        ...
        val = expr.subs(subs_dict).n()  # .n() is Sage's numerical evaluation
        return not np.isnan(val) and not np.isinf(val)
    except Exception:
        return False
...
def check_analytical_properties(density_str, free_consts_names):
    """ Given a density profile as a string, checks various analytical properties."""
    # --- Setup ---
    var('r, rp, G')
    assume(r > 0)
    assume(rp > 0)
    ...
    for name in free_consts_names:
        ...
        # alpha usually controls sharpness of transition (must be > 0)
        if name == 'alpha':
            assume(v > 0)
            
        # beta usually controls outer slope (must be > 3 for finite mass, > 2 for finite potential)
        # Let's assume > 0 to start (often > 3 is needed for convergence, so to check)
        if name == 'beta':
            assume(v > 0) 
            
        # gamma controls inner slope (cusp). 
        # Must be < 3 for finite mass at origin. 
        # Typically 0 <= gamma < 3.
        if name == 'gamma':
            # assume(v > -1)
            assume(v < 3)
    ...
    try:
        ...
        symb, num = False, False
    results.append({'Property': 'Potential (wo ref. pt.)', 'symb_condition': symb, 'num_condition': num, 'symb_result': Phi_inf})
